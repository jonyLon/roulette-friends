...
export class RoulettePhysics{
 constructor(launch){
  ...
  for(const d of GEOMETRY.deflectors){const c=this.world.createCollider(RAPIER.ColliderDesc.cuboid(.036,.022,.024).setTranslation(...d.position).setRotation(quat(d.rotation)).setFriction(.18).setRestitution(.48).setActiveEvents(RAPIER.ActiveEvents.COLLISION_EVENTS));this.deflectorHandles.add(c.handle);}
 ...
 proof(){return {...this.frame(),ballSpeed:length(this.ball.linvel()),ballAngularSpeed:length(this.ball.angvel()),wheelSpeed:length(this.wheel.angvel()),stable:this.stable,engine:ENGINE_VERSION};}
